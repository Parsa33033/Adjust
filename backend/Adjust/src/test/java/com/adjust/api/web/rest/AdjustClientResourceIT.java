...
public class AdjustClientResourceIT {
    ...
    @Mock
    private AdjustClientRepository adjustClientRepositoryMock;

    @Autowired
    private AdjustClientMapper adjustClientMapper;

    @Mock
    private AdjustClientService adjustClientServiceMock;
    ...
    public void getAllAdjustClientsWithEagerRelationshipsIsEnabled() throws Exception {
        when(adjustClientServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAdjustClientMockMvc.perform(get("/api/adjust-clients?eagerload=true"))
            .andExpect(status().isOk());

        verify(adjustClientServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllAdjustClientsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(adjustClientServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

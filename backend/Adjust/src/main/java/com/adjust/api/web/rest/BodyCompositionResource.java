...
public class BodyCompositionResource {
    ...
    public List<BodyCompositionDTO> getAllBodyCompositions(@RequestParam(required = false) String filter) {
        if ("medicalprofile-is-null".equals(filter)) {
            log.debug("REST request to get all BodyCompositions where medicalProfile is null");
            return bodyCompositionService.findAllWhereMedicalProfileIsNull();
        }
        if ("adjustprogram-is-null".equals(filter)) {
            log.debug("REST request to get all BodyCompositions where adjustProgram is null");
            return bodyCompositionService.findAllWhereAdjustProgramIsNull();
        }

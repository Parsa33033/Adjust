...
export const AdjustClientUpdate = (props: IAdjustClientUpdateProps) => {
  const [idstutorials, setIdstutorials] = useState([]);
  const [medicalProfileId, setMedicalProfileId] = useState('0');
  const [isNew, setIsNew] = useState(!props.match.params || !props.match.params.id);

  const { adjustClientEntity, medicalProfiles, tutorials, loading, updating } = props;
  ...
  const saveEntity = (event, errors, values) => {
    if (errors.length === 0) {
      const entity = {
        ...adjustClientEntity,
        ...values,
        tutorials: mapIdList(values.tutorials),
      };
        ...
        <Col md="8">
          {loading ? (
            ...
            <AvForm model={isNew ? {} : adjustClientEntity} onSubmit={saveEntity}>
              ...
              <AvGroup>
                <Label for="adjust-client-medicalProfile">
                  <Translate contentKey="adjustApp.adjustClient.medicalProfile">Medical Profile</Translate>
                </Label>
                <AvInput id="adjust-client-medicalProfile" type="select" className="form-control" name="medicalProfileId">
                  <option value="" key="0" />
                  {medicalProfiles
                    ? medicalProfiles.map(otherEntity => (
              ...
              <AvGroup>
                <Label for="adjust-client-tutorials">
                  <Translate contentKey="adjustApp.adjustClient.tutorials">Tutorials</Translate>
                </Label>
                <AvInput
                  id="adjust-client-tutorials"
                  type="select"
                  multiple
                  className="form-control"
                  name="tutorials"
                  value={adjustClientEntity.tutorials && adjustClientEntity.tutorials.map(e => e.id)}
                >
                  <option value="" key="0" />
                  {tutorials
                    ? tutorials.map(otherEntity => (
                        <option value={otherEntity.id} key={otherEntity.id}>
                          {otherEntity.id}
                        </option>
                      ))

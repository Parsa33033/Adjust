            ...
            <tbody>
              {adjustClientList.map((adjustClient, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {adjustClient.medicalProfileId ? (
                      <Link to={`medical-profile/${adjustClient.medicalProfileId}`}>{adjustClient.medicalProfileId}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {adjustClient.tutorials
                      ? adjustClient.tutorials.map((val, j) => (
                          <span key={j}>
                            <Link to={`tutorial/${val.id}`}>{val.id}</Link>
                            {j === adjustClient.tutorials.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
